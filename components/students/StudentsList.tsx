      ...
      <div className="grid grid-cols-1 gap-3 sm:gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {isLoading ? (
          ...
          filteredStudents.map((student) => (
            <Card key={student.id} className="p-3 sm:p-6 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
              <div className="flex items-start space-x-3 sm:space-x-4">
                <Avatar className="h-10 w-10 sm:h-12 sm:w-12 bg-blue-100 flex-shrink-0">
                  <AvatarImage src={student.image} />
                  <AvatarFallback className="text-blue-600 font-medium text-sm sm:text-base">
                      ...
                      <AlertDialog>
                        <AlertDialogTrigger asChild>
                          <Button 
                            variant="ghost" 
                            size="icon"
                            className="h-7 w-7 sm:h-8 sm:w-8 text-gray-500 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20"
                          >
                            <UserMinus className="h-3.5 w-3.5 sm:h-4 sm:w-4" />
                          </Button>
                        </AlertDialogTrigger>
                        <AlertDialogContent className="sm:max-w-[425px] bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 mx-4 sm:mx-auto">
                          <AlertDialogHeader className="bg-white dark:bg-gray-900">
                            <AlertDialogTitle className="text-lg sm:text-xl text-gray-900 dark:text-gray-100">Disconnetti Studente</AlertDialogTitle>
                          ...
                          <AlertDialogFooter className="gap-2 sm:gap-3">
                            <AlertDialogCancel className="mt-0 px-3 sm:px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600 dark:hover:bg-gray-700">
                              Annulla
                            </AlertDialogCancel>
                            <AlertDialogAction
                              onClick={() => handleDisconnectStudent(student)}
                              className="px-3 sm:px-4 py-2 text-sm font-medium text-white bg-red-600 rounded-lg hover:bg-red-700 dark:bg-red-700 dark:hover:bg-red-800"
                            >
                              Disconnetti
                            </AlertDialogAction>
                          </AlertDialogFooter>
